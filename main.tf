...
resource "azurerm_key_vault_access_policy" "default" {
  ...
  key_permissions         = "${lookup(var.policy_object_list_map[count.index],"key_permissions")}"
  secret_permissions      = "${lookup(var.policy_object_list_map[count.index],"secret_permissions")}"
  certificate_permissions = "${lookup(var.policy_object_list_map[count.index],"certificate_permissions")}"
}
